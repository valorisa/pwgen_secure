...
def load_wordlist(path: str):
    ...
    if not p.exists():
        print(
            f"Warning: wordlist '{path}' non trouvée — "
            "utilisation de la wordlist embarquée."
        )
        return DEFAULT_WORDS.copy()
    words: list[str] = []
...
def gen_passphrase(min_chars: int, words: list, separator: str = " ") -> str:
    ...
    Assure une longueur minimale (MIN_LENGTH) et au moins 3 mots.
    """
    # ensure minimum length using builtin max
    min_chars = max(min_chars, MIN_LENGTH)
    chosen: list[str] = []
    while True:
...
def prompt_int(prompt: str, default: int) -> int:
    """Invite l'utilisateur pour un entier; retourne la valeur par défaut si entrée vide."""
...
def choose_separator():
    ...
    while True:
        ...
        if 1 <= idx <= len(defaults):
            ...
            if val is None:
                custom = input(
                    "Saisir le séparateur souhaité (ou '<space>' pour espace) : "
                )
                if "<space>" in custom:
...
def handle_generate(single: bool):
    """Gère l'interface pour génération simple ou en batch."""
    try:
        ...
        exclude_prompt = (
            'Caractères spéciaux à EXCLURE (tapez tels quels; utilisez "<space>" '
            "pour espace; laisser vide = aucun): "
        )
        exclude = input(exclude_prompt).strip()
        if "<space>" in exclude:
            exclude = exclude.replace("<space>", " ")

        if single:
            try:
                pwd = gen_password_exact_counts(
                    length,
                    exact_lower,
                    exact_upper,
                    exact_digits,
...
def handle_passphrase():
    """Gère la génération de phrase de passe et l'affichage."""
    try:
        ...
        if sep is None:
            ...
            return
        wordlist_path = input(
            "Chemin vers wordlist (une ligne = un mot) ou laisser vide pour default: "
        ).strip()
        ...
        # print length and word count on separate lines to remain under 100 cols
        print("Longueur:", len(passphrase))
        print("Nombre de mots:", len(passphrase.split(sep)))
        print()
    except ValueError:
...
def show_rules():
    """Affiche les règles et exemples à l'utilisateur."""
    print("\nRègles / exemples :")
    print(f"- Longueur minimale enforceée : {MIN_LENGTH}")
    print("- Pour exclure l'espace, tapez '<space>' dans le champ d'exclusion.")
    print(
        "- Si la somme des EXACTS dépasse la longueur, une erreur sera affichée."
    )
    print(
        "- Si vous excluez tous les caractères spéciaux mais demandez des "
        "spéciaux > 0, cela provoquera une erreur."
    )
    print(
        "- Pour la phrase de passe, fournissez une wordlist (fichier texte) si "
        "vous voulez de meilleurs mots; sinon la wordlist embarquée sera utilisée."
    )
    print(
        "- Le menu de séparateurs propose quelques caractères Thaï par défaut; "
        "vous pouvez remplacer/étendre cette liste dans la fonction "
        "choose_separator()."
    )
    print()
